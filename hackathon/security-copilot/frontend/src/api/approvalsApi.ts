...
import { Approval, ApprovalListResponse, ApprovalListParams } from '../types/approval';

export const approvalsApi = {
  approve: async (articleId: number, notes?: string): Promise<Approval> => {
    const response = await apiClient.post<Approval>(`/approvals/${articleId}/approve`, { notes });
    return response.data;
  },

  reject: async (articleId: number, notes?: string): Promise<Approval> => {
    const response = await apiClient.post<Approval>(`/approvals/${articleId}/reject`, { notes });
    return response.data;
  },

  underReview: async (articleId: number, notes?: string): Promise<Approval> => {
    const response = await apiClient.post<Approval>(`/approvals/${articleId}/under-review`, { notes });
    return response.data;
  },
